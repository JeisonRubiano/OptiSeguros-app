...
// Estados de póliza disponibles
const ESTADOS_POLIZA = [
    { value: '', label: 'PENDIENTE', color: 'default' },
    { value: 'RECAUDADA', label: 'RECAUDADA', color: 'green' },
    { value: 'ANULADA', label: 'ANULADA', color: 'red' }
];

export default function DataTable({ data, title, onRowClick, onEstadoChange }) {
        ...
        const loadStates = async () => {
            try {
                ...
                if (result.success && result.data) {
                    ...
                    const states = {};
                    Object.keys(result.data).forEach(consecutivo => {
                        states[consecutivo] = result.data[consecutivo].estado;
                    });
                    setRowStates(states);
                }
            } catch (error) {
                console.error('Error loading policy states:', error);
            }
        };
        loadStates();
    }, []);
    ...
    const saveStateToBackend = async (consecutivo, estado) => {
        try {
            await fetchWithAuth(`/api/policy-states/save`, {
                method: 'POST',
                body: JSON.stringify({
            ...
            console.error('Error saving policy state:', error);
        }
    };
    ...
    if (!data || data.length === 0) {
        return (
            <div className="p-12 text-center bg-slate-50 rounded-xl border border-slate-200">
                ...
                <p className="text-slate-500 mt-1">Seleccione un archivo diferente o verifique la conexión.</p>
            </div>
        );
    }
    ...
    const handleEstadoChange = (rowIndex, consecutivo, newEstado) => {
        const key = consecutivo || `row_${rowIndex}`;
        setRowStates(prev => ({
            ...prev,
            [key]: newEstado
        }));
        ...
        if (consecutivo) {
            saveStateToBackend(consecutivo, newEstado);
        }

        if (onEstadoChange) {
            onEstadoChange(key, newEstado);
        }
    };

    // Obtener el estado actual de una fila
    const getRowEstado = (row, index) => {
        const key = row.CONSECUTIVO || `row_${index}`;
        return rowStates[key] || row.ESTADO || '';
    };

    // Obtener clases de color para la fila según el estado
    const getRowColorClass = (estado) => {
        const estadoUpper = String(estado).toUpperCase();
        if (estadoUpper === 'RECAUDADA') {
        ...
        return 'hover:bg-emerald-50/50'; // Default
    };

    // Calculate Total
    const totalAmount = filteredData.reduce((sum, row) => {
        let primaCol = columns.find(c => String(c).toUpperCase().includes("PRIMAS USD"));
        if (!primaCol) {
            primaCol = columns.find(c => String(c).toUpperCase().includes("VALOR PRIMA TOTAL DOLARES"));
        }
        if (!primaCol) {
            primaCol = columns.find(c => {
                const upper = String(c).toUpperCase();
                return (upper.includes("PRIMA") || upper.includes("VALOR") || upper.includes("USD")) &&
                    !upper.includes("FECHA") &&
                    !upper.includes("ESTADO") &&
                    !upper.includes("CONSECUTIVO");
            });
        }
        if (primaCol) {
    ...
    const estadoCounts = filteredData.reduce((acc, row, idx) => {
        ...
        const estadoUpper = String(estado).toUpperCase();
        if (estadoUpper === 'RECAUDADA') acc.recaudadas++;
        else if (estadoUpper === 'ANULADA') acc.anuladas++;
        else acc.pendientes++;
        return acc;
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                    ...
                    <thead className="bg-slate-100 text-slate-700 font-bold uppercase tracking-wider text-xs border-b-2 border-slate-300 sticky top-0 z-10">
                        <tr>
                            ...
                            {columns.map((col) => {
                                ...
                                const filterableColumns = ['ESTADO', 'REGIONAL', 'Sucursal', 'PRODUCTO', 'PERIODICIDAD', 'FORMA COBRO'];
            ...
            <div className="bg-slate-50 px-8 py-4 border-t border-slate-100 flex justify-between items-center">
                <div className="flex items-center gap-4 text-xs text-slate-500">
                    ...
                    <span className="flex items-center gap-1">
                        <span className="w-3 h-3 bg-white border border-slate-200 rounded"></span> Pendiente
                    </span>
                </div>
